...
inline vector<string> grid(int n) {
    vector<string> g(n);
    for (auto& row : g) {
        cin >> row;
    }
...
inline vector<string> padded_grid(int n, int m, char pad = '#') {
...
inline vector<pair<int, int>> edges(int m, int input_base = 1, int output_base = 0) {
...
inline vector<vector<int>> graph(
    int n,
    int m,
    bool directed = false,
    int input_base = 1,
