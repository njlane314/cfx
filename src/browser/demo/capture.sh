...
cp "$repo_root/assets/templates/solution.cpp" "$work/root/templates/solution.cpp"
cp -R "$repo_root/assets/include/cp" "$work/root/include/cp"

browser_environment=(
    "CFX_STATE_ROOT=$work/state"
...
fetch=$(
    env "${browser_environment[@]}" \
        ...
        "$repo_root/cfx" --root "$work/root" 71A
)
cp "$script_dir/fixture/solution.cpp" "$work/root/codeforces/71/A/solution.cpp"
submit=$(
    cd "$work/root"
    env "${browser_environment[@]}" CFX_STD=c++20 \
        "CFX_API_BASE=file://$script_dir/fixture/api" \
        "$repo_root/cfx" --root "$work/root" submit
)
