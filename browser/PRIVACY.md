...
The cf-probs connector acts only when the local `probs` command opens a
...
- the resulting Codeforces submission URL and verdict.

Problem data travels from Codeforces to the `probs` process through a
...
by `probs` remain in the user's local workbench; Codeforces retains submitted
...
[cf-probs issue tracker](https://github.com/njlane314/cf-probs/issues). Local
files can be deleted directly by the user; this project holds no server-side
copy.
